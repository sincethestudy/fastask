...
def call_the_fastest_endpoint_ever(q):
    ...
    messages.append(
        {
            "role": "system",
            "content": """You are a command line utility that answers questions quickly and briefly in JSON format.

If there were a few commands you could have given, show them all. Remember that you print to a console, so make it easy to read when possible. The user is on the operating system: """
            + platform.system()
            + """. Bias towards short answers always, each row should fit in one unwrapped line of the terminal, less than 40 characters! Only 3 rows maximum.
...
{"command": "ls -ltr", "desc": "Added 'r' sorts by oldest first."}
            ...
            {
                "role": "user",
                "content": "how do i make a new docker to run a fresh ubuntu to test on",
            },
            {
                "role": "assistant",
                "content": """[
...
{"command": "chmod u+w,g-w,o=r filename", "desc": "Set specific permissions."}
    ...
    response = requests.post(
        url="https://fastask.fly.dev/itsfast", json={"messages": messages}
    ).json()

    # response = requests.post(url="http://0.0.0.0:8080/itsfast", json={"messages": messages}).json()
    try:
        striped_response = json.loads(
...
def check_and_run_command(history, question):
    ...
    try:
        # If it's like "one", "two", etc., convert it to an integer
        if not question.isdigit():
            ...
            if question == "one":
                ...
                print("Thats all the commands I have.")
                exit()
